"""
dashboard/server.py — Karen Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "Karen Uploads",
        Path.home() / "Documents" / "Karen Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"Karen Dashboard Port {port}"
        prog_rule  = "Karen Dashboard Python"
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until Karen is run as Administrator.")
...
def _ensure_ssl_certs() -> bool:
    ...
    try:
        ...
        subject = issuer = x509.Name([
            x509.NameAttribute(NameOID.COMMON_NAME, "Karen Local Remote"),
        ])
        alt_names = [
...
<p>Press <strong style="color:#dde3ed">Mobile Connect</strong> in Karen to get a new QR code.</p>
</div></body></html>""")
...
<p>Connecting to Karen…</p>
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Mobile Connect' in Karen UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
